 ...
 *
 */
public class Nikola_Antic_Test {
    ...
    public static String abeceda(String nizSlova) {
        ...
        String slovoKojeFali="";
        for (int i = 0; i < alfabet.length(); i++) {
            boolean imaSlovo=false;
            for (int j = 0; j <nizSlova.length(); j++) {
                if (alfabet.charAt(i)==nizSlova.charAt(j)) {
                    imaSlovo = true;
                    break;
                }
            }
                if (!imaSlovo) {
                    slovoKojeFali+=alfabet.charAt(i);
            }
        }
        return slovoKojeFali;
    }
    public static void main(String[] args) {
        //System.out.println(Arrays.toString(lowerCaseNiz(new String[]{"Stefan","Dusan","Ana"})));
        //System.out.println(indexGdeElementPripada(4,new int[]{1,2,3,5,6}));
        System.out.println(abeceda("BDTORUXZYWQACEF"));
    }
}
